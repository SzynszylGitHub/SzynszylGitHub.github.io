...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateData(nodeId) {
        // Zmiana zaznaczenia w Menu górnym
        listItems.forEach(li => li.classList.remove('active'));
        const activeNav = document.querySelector(`.module-list li[data-target="${nodeId}"]`);
        if(activeNav) activeNav.classList.add('active');

        const data = projectData[nodeId];
        if(!data) return;

        // Zanikająca animacja przejścia
        contentWrapper.style.opacity = 0;
        
        setTimeout(() => {
            // Aktualizacja sekcji - przeniesiony tytuł
            document.getElementById('detail-main-title').innerText = data.mainTitle.toUpperCase();
        ...
        }, 300); // Odpowiada za opóźnienie zanikania z CSS
    }
